# Recall that the perfect discriminator D = p{data}/(p{data}+p{g}) in the original GAN paper.
# p{data} = p{g}*D/(1-D) When the generator is optimal, p{g} = p{data} and D = 0.5.
# Thus, we want to generate x~G(z) such that D(x) is close to the decision boundary 0.5.
# The new objective for the generator could be minimizing L(G) = 0.5*(log(D(G(z))) - log(1-D(G(z))))^2.
...
def train(n_epoch, batch_size, gpu = False, save_path = './bsgan/'):
	# check path
	...
	for epoch_idx in range(n_epoch):
		for batch_idx, (data, _) in enumerate(train_loader):
			...
			# train the generator -- minimize L(G) = 0.5*[log(D(G(z))) - log(1-D(G(z)))]^2
			G.zero_grad()
			out = D(g_sample)
			G_loss = 0.5*torch.mean((torch.log(out)-torch.log(1-out))**2)
			G_loss.backward()
			optimizerG.step()
