...
def train(n_epoch, batch_size, gpu = False, save_path = './wgan/'):
	# check path
	...
	# build model
	G, D, optimizerG, optimizerD = build_graph(gpu = gpu)
	...
	for epoch_idx in range(n_epoch):
		for batch_idx, (data, _) in enumerate(train_loader):
			...
			for _ in range(5):
				...
				# train the discriminator --minimize L(D) = -[f(D(x)) + f1-D(G(z)))]
				D.zero_grad()
				D_loss = -(torch.mean(real) - torch.mean(fake))
				D_loss.backward()
				optimizerD.step()
				# weight clipping
				for layer in D.parameters():
					layer.data.clamp_(-0.01, 0.01)
